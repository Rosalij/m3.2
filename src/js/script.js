...
let url = "https://m3-1b6e.onrender.com/experiences"
//ul element from DOM
...
async function getData() {
    ...
    if (data) {
        data.forEach(exp => {
            ...
            deleteEl.addEventListener("click", () => { deleteData(exp._id, liEl) })
        });
    }
};
...
async function deleteData(id, liEl) {
    try { //fetch API with method DELETE, delete experience with chosen ID
        const response = await fetch(`https://m3-1b6e.onrender.com/experiences/${id}`, {
            method: 'DELETE',
            headers: {
